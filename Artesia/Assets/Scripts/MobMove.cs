...
public class MobMove : MonoBehaviour, IState<MobController>
{
    ...
    public void OperateEnter(MobController sender){
        if(!mobController)
            mobController = sender;

        elapsedTime = 0;
        do{
            dir.x = Random.Range(-1,2);
            dir.y = Random.Range(-1,2);
            hit = Physics2D.Raycast(mobController.transform.position, dir, 1, LayerMask.GetMask("Tile"));
        } while(hit);

        Vector3 startPos = new Vector3((int)mobController.transform.position.x, (int)mobController.transform.position.y,0);
        targetPos = startPos + new Vector3(dir.x, dir.y, 0);
    }
    public void OperateUpdate(MobController sender){
        Vector3 nowPos = mobController.transform.position;
        mobController.transform.position = Vector2.Lerp(nowPos, targetPos, elapsedTime / speed);
        elapsedTime += Time.deltaTime;
        if(elapsedTime >= speed){
            mobController.transform.position = targetPos;
            // state 변경 ??
            // 아니면 statemachine에서
        }
    }
